...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：请求行（请求方法、请求 URI、协议版本）
   请求头（多个键值对，描述客户端信息等）
   空行（分隔请求头与请求体）
   请求体（携带 POST 等方法的提交数据）

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404：Not Found，服务器找不到请求的资源
500：Internal Server Error，服务器内部处理请求时出错
503：Service Unavailable，服务器暂时无法处理请求

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET：参数在 URL 中，长度受限，可缓存，安全性低
POST：参数在请求体中，长度无限制，不可缓存，安全性高

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：区别：
HTTP：明文传输，端口 80，无加密
HTTPS：加密传输，端口 443，基于 SSL/TLS
保护机制：
身份认证：验证服务器身份
数据加密：对传输内容加密
完整性校验：防止数据被篡改
...
5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？
   ...
   > 答：HTTPS 加密是客户端与服务器之间的传输加密，浏览器开发者工具运行在客户端，能在数据加密前 / 解密后直接查看明文内容，并非在传输链路上解密
