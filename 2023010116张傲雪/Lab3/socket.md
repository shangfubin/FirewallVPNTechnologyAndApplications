...
| 连接服务器的方法名 |connect() |
| 发送数据的方法名 |send() |
| 接收数据的方法名（含参数） |recv(bufsize) |
| 断开连接的方法名 |close() |

**嵌入截图：**
运行结果截图![alt text](run.png)
...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |listen | 连接建立阶段|
| `client.py` 已连接，数据尚未发送完毕 | establish|数据传输阶段 |
| `server.py` 执行 `close()` 之后 | FIN_WAIT2|连接释放阶段 |

**嵌入截图（LISTEN 状态）：**

ss LISTEN 截图![alt text](ss_listen.PNG)

**嵌入截图（ESTABLISHED 状态）：**

ss ESTABLISHED 截图![alt text](ss_established.PNG)
...
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 |调用close()触发的断开 |

**嵌入截图：**

http_client.py 运行结果![alt text](http_run.PNG)
---
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 |否 |
| `ss` 中连接的 State | 不存在（已关闭）|established（连接仍保持） |
| 需要手动 Ctrl+C 才能结束吗 |不需要 |需要 |

**嵌入截图（HTTP/1.1 卡住时的 ss 输出）：**

ss keepalive 截图![alt text](ss_keepalive.PNG)
---
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：
   报错：由于目标计算机积极拒绝，无法连接，此时服务器套接字不存在因为服务器未启动，管道同样不存在因为服务器和客户端的三次握手无法完成。


2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：
   accept() 返回一个新的套接字，而不是复用原来的 server 套接字，是为了将"监听"和"通信"两个职责分开。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：
   描述符是操作系统内核标识本地 I/O 对象的整数，解决进程如何识别套接字的问题；端口号是 TCP/IP 协议中标识网络服务的编号，解决外部主机如何找到对应服务的问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：
   客户端没有指定端口号，端口号由操作系统自动分配，在'ss'输出中能看到，显示在 Local Address 列。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：
   为 HTTP/1.0 默认使用短连接，服务器发送完响应后主动关闭了连接，导致客户端的 recv() 收到 b''（空数据），从而退出循环

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：
   HTTP/1.0 默认短连接，服务器发送完响应后主动关闭，而HTTP/1.1 默认开启长连接（keep-alive），服务端不主动关闭连接，所以 recv() 阻塞等待、连接保持 ESTABLISHED。
